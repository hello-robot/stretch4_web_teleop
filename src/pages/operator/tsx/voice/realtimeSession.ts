    ...
    if (fc.name === EXECUTE_BASE_MOVE) {
        ...
        try {
            const parsed = JSON.parse(fc.arguments || "{}") as Record<
                ...
                unknown
            >;
            argsSummary = JSON.stringify({ name: parsed.name });
        } catch {
            //
        }
    }
...
export async function connectOpenAIRealtimeVoice(
    ...
    > = {
        ...
        save_pose: async (voiceProvider, fc) => {
            const res = await executeSavePose(fc.arguments, voiceProvider);
