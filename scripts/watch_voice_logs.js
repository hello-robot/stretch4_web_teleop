...
function formatAndPrintLine(line) {
    ...
    try {
        ...
        const timeShort = record.timestamp
        ...
        if (record.message || record.type === "log") {
            ...
            console.log(
                `${chalk.grey(`[VoiceLog ${timeShort}]`)} ${formatted}`
            );
            return;
        }

        // 2. Structured voice interaction summary record
        const transcriptText = record.input?.transcript
        ...
        const toolText = record.output?.tool_name
            ...
            : "(no tool)";
        const statusIcon = record.execution?.success
            ? chalk.green("✅ SUCCESS")
            : chalk.red("❌ REJECTED");
        const detailText = record.execution?.detail
